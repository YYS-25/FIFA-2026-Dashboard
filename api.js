...
function parseCupFinalsText(cupFinalsText, matchStartIndex = 49) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (matchData) {
      ...
      // or, after extra time/penalties: "Germany 1-1 a.e.t. (1-1, 0-1), 3-4 pen. Paraguay"
      let homeGoals = null;
      let awayGoals = null;
      let status = "upcoming";
      ...
      const aetRegex =
        /^(.+?)\s+(\d+)-(\d+)\s+a\.e\.t\.\s+\(\d+-\d+,\s*\d+-\d+\)\s*,\s*(\d+)-(\d+)\s+pen\.\s+(.+?)\s*$/;
      const scoreRegex = /^(.+?)\s+(\d+)-(\d+)\s+(?:\(\d+-\d+\)\s+)?(.+?)\s*$/;
      const aetMatch = teamInfo.match(aetRegex);
      const scoreMatch = aetMatch ? null : teamInfo.match(scoreRegex);

      let home, away;
      if (aetMatch) {
        home = aetMatch[1].trim();
        homeGoals = parseInt(aetMatch[2], 10);
        awayGoals = parseInt(aetMatch[3], 10);
        penaltyScore = `${aetMatch[4]}-${aetMatch[5]} pen.`;
        away = aetMatch[6].trim();
        status = "completed";
      } else if (scoreMatch) {
...
async function fetchWorldCupMatches() {
  try {
    ...
    if (cupFinalsText) {
      const knockoutMatches = parseCupFinalsText(cupFinalsText, 73);
    ...
    if (cupFinalsText) {
      ...
      Object.entries(knockoutMatches).forEach(([matchId, match]) => {
        if (matches[matchId]) {
          ...
          matches[matchId].penaltyScore = match.penaltyScore;
